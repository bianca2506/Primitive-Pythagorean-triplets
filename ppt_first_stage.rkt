...
(define (multiply-helper M V result)
  (if (null? M)
      (reverse result) ; inversez rezultatul
      (multiply-helper (cdr M) V (cons (dot-product (car M) V) result))))
; adaug in rezultat produsul intre primul rand a lui M si V
; si cu ajutorul lui cdr ajung la urmatoarele randuri ale lui M

(define (multiply M V)
  (multiply-helper M V null)) ; apelez functia helper
...
; la aceasta functie am folosit recursivitate pe coada deoarece
; calculez rezultatul in ppt care este pe coada
