...
(define (key n)
  (let* ((e (cadr (get-nth-quadruple n)))
         ...
         ;(a2 (a2 e f)) (b2 (b2 e f)) (c2 (c2 e f))
         ;(a3 (a3 e f)) (b3 (b3 e f)) (c3 (c3 e f)))
    (map (λ (x) (modulo (x e f) 27)) (list a1 b1 c1 a2 b2 c2 a3 b3 c3)))))
...
(define (message->codes message)
  (let* ((stringToList (string->list message))
         (charToInteger (map char->integer stringToList)))
    (let iter ((charToInteger charToInteger) (result null))
      (cond
        [(null? charToInteger) (reverse result)]
        [(= (car charToInteger) 32) (iter (cdr charToInteger) (cons 0 result))]
        [else  (iter (cdr charToInteger) (cons (- (car charToInteger) 96) result))]))))
...
(define (codes->message codes)
  (list->string (map integer->char
                     (let iter ((codes codes) (result null))
                       (cond
                         [(null? codes) (reverse result)]
                         [(= (car codes) 0) (iter (cdr codes) (cons 32 result))]
                         [else (iter (cdr codes) (cons (+ (car codes) 96) result))])))))
...
(define (truncate key size)
  (if (= size 0) null
      (cons (car key) (truncate (cdr key) (sub1 size)))))

(define (extend-key key size)
  (if (< size (length key))
      (truncate key size)
      (let concatenate ((key key) (size size))
        (if (= (quotient size (length key)) 1)
            (append key (truncate key (modulo size (length key))))
            (append key (concatenate key (- size (length key))))))))
...
(define mod (λ (x) (modulo x 27)))
(define function (λ (x y z) (map x y (extend-key z (length y)))))

(define (encrypt-codes message key) 
  (map mod (function + message key)))

(define (decrypt-codes crypted key)
  (map mod (function - crypted key)))
...
(define (encrypt-message message key)
  (codes->message (map mod (function + (message->codes message) key))))

(define (decrypt-message crypted key)
  (codes->message (map mod (function - (message->codes crypted) key))))
